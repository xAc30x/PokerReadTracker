...
const EMPTY_STATE: TrackerState = {
  players: [], seats: [], counts: {}, hudStats: {}, recentHands: [],
...
const PREFLOP: Action[] = [
  { id: "fold", label: "Fold", short: "F" }, { id: "limp", label: "Limp", short: "L" },
  { id: "call", label: "Call", short: "C" }, { id: "open-raise", label: "Open", short: "OR" },
  { id: "three-bet", label: "3-Bet", short: "3B" }, { id: "four-bet-plus", label: "4-Bet+", short: "4+" },
  { id: "squeeze", label: "Squeeze", short: "SQ" }, { id: "all-in", label: "All-In", short: "AI" },
];
const POSTFLOP: Action[] = [
  { id: "check", label: "Check", short: "X" }, { id: "bet", label: "Bet", short: "B" },
  { id: "call", label: "Call", short: "C" }, { id: "postflop-raise", label: "Raise", short: "R+" },
  { id: "postflop-fold", label: "Fold", short: "F" }, { id: "check-raise", label: "Check-Raise", short: "XR" },
  { id: "donk-bet", label: "Donk", short: "DB" }, { id: "postflop-all-in", label: "All-In", short: "AI" },
];
const SHOWDOWN: Action[] = [
  { id: "bluff-shown", label: "Bluff", short: "BL" }, { id: "value-shown", label: "Value", short: "V" },
  { id: "draw-shown", label: "Draw", short: "DR" }, { id: "mucked-unknown", label: "Muck", short: "?" },
];
const POSITIONS = {
...
function optimistic(state: TrackerState, mutation: TrackerMutation): TrackerState {
  if (mutation.type === "addObservation") {
    const byPlayer = state.counts[mutation.playerId] ?? {};
    const byPhase = byPlayer[mutation.phase] ?? {};
  ...
  if (mutation.type === "advanceHand") {
    return { ...state, table: { ...state.table, handNumber: state.table.handNumber + 1, positionOffset: (state.table.positionOffset + 1) % state.table.tableSize, currentHandId: mutation.id } };
  }
  return state;
}
...
export function CompanionHud() {
  ...
  const queue = useRef<TrackerMutation[]>([]);
  ...
  useEffect(() => {
    ...
    setMeta({ ...DEFAULT_META, ...stored, playerMeta: stored.playerMeta ?? {}, streetLogs: stored.streetLogs ?? [] });
    queue.current = readLocal<TrackerMutation[]>(QUEUE_KEY, []);
  ...
  const seated = useMemo(() => data.seats.slice().sort((a, b) => a.seatNo - b.seatNo)
    .map((seat) => ({ seat, player: byId.get(seat.playerId) }))
    .filter((x): x is { seat: { seatNo: number; playerId: string }; player: Player } => Boolean(x.player)), [byId, data.seats]);
  ...
  useEffect(() => { if (!meta.selectedId && selectedId) saveMeta((m) => ({ ...m, selectedId })); }, [meta.selectedId, saveMeta, selectedId]);
  ...
  function log(phase: Phase, action: string, label: string) {
    ...
    let streetLogId: string | undefined;
    if (phase === "postflop") {
  ...
  return <main className={`hud2${meta.gameMode ? " hud2--game" : ""}`}>
    <header className="hud2__topbar"><div><span className="hud2__eyebrow">TableRead · iOS HUD</span><strong>{meta.sessionMode === "cash" ? "Cash session" : "Tournament"}</strong></div>
      <button type="button" className={`hud2__sync hud2__sync--${sync}`} onClick={() => void flush().then((ok) => ok && load())}>{sync === "loading" ? "Loading" : sync === "saving" ? "Saving" : sync === "queued" ? "Offline queue" : sync === "error" ? "Retry" : "Live"}</button></header>

    <section className="hud2__session-bar"><div className="hud2__segment">{(["cash", "tournament"] as const).map((mode) => <button key={mode} type="button" className={meta.sessionMode === mode ? "is-active" : ""} onClick={() => saveMeta((m) => ({ ...m, sessionMode: mode }))}>{mode === "cash" ? "Cash" : "Tournament"}</button>)}</div>
      <button type="button" className="hud2__game-toggle" aria-pressed={meta.gameMode} onClick={() => saveMeta((m) => ({ ...m, gameMode: !m.gameMode }))}>{meta.gameMode ? "Exit game mode" : "Game mode"}</button></section>
    ...
    <nav className="hud2__players" aria-label="Fast player switcher">{seated.length ? seated.map(({ seat, player: p }) => {
      const pm = meta.playerMeta[p.id]; return <button key={p.id} type="button" className={p.id === selectedId ? "is-active" : ""} data-color={pm?.color ?? "neutral"} onClick={() => saveMeta((m) => ({ ...m, selectedId: p.id }))}>
        <span className="hud2__avatar">{initials(p.name)}</span><span className="hud2__player-copy"><strong>{p.name}</strong><small>{seatPosition(seat.seatNo, data)}{pm?.stack ? ` · ${pm.stack}` : ""}</small>{pm?.wallet ? <em>Wallet {pm.wallet}</em> : null}</span></button>;
    }) : <div className="hud2__empty">No players are seated. <a href="/">Open full table setup</a>.</div>}</nav>

    {player ? <>
      <section className="hud2__player-head" data-color={local.color}><div className="hud2__avatar hud2__avatar--large">{initials(player.name)}</div><div><span>{position || "Player pool"}</span><h1>{player.name}</h1><p>{player.playStyle.toUpperCase()} · {hud?.sampleHands ?? 0} observed hands</p></div>
        <div className="hud2__mini-stats"><span><b>{pct(hud?.vpipPct)}</b>VPIP</span><span><b>{pct(hud?.pfrPct)}</b>PFR</span><span><b>{pct(hud?.threeBetPct)}</b>3B</span></div></section>

      <section className="hud2__local-meta hud2__setup-only"><label>Stack<input inputMode="decimal" placeholder="e.g. 82 BB" value={local.stack} onChange={(e) => patchPlayer({ stack: e.target.value.slice(0, 24) })} /></label>
        <label>Wallet<input inputMode="decimal" placeholder="$ / chips" value={local.wallet} onChange={(e) => patchPlayer({ wallet: e.target.value.slice(0, 24) })} /></label>
        <div className="hud2__colors">{COLORS.map((color) => <button key={color} type="button" data-color={color} className={local.color === color ? "is-active" : ""} aria-label={`Tag ${player.name} ${color}`} onClick={() => patchPlayer({ color })} />)}</div></section>
      ...
      <section className="hud2__block hud2__street-block"><div className="hud2__street-tabs">{(["flop", "turn", "river"] as const).map((street) => <button key={street} type="button" className={meta.street === street ? "is-active" : ""} aria-selected={meta.street === street} onClick={() => saveMeta((m) => ({ ...m, street }))}>{street}</button>)}</div>
        <div className="hud2__actions">{POSTFLOP.map((a) => <button key={a.id} type="button" onClick={() => log("postflop", a.id, a.label)}><b>{a.short}</b><span>{a.label}</span><small>{streetCount(a.id)}</small></button>)}</div></section>
    ...
    <footer className="hud2__footer"><a href="/">Full tracker</a><span>Manual observation HUD · no solver/RTA</span></footer>
